...
## Getting sneaky with IEEE 754 {#ieee}
...
Note that we _should_ add some kind of check in the constructor to ensure our `OrderedFloat` isn't being built with a negative number, but for the sake of this blog post I'll skip that.

## Benchmark {#benches}
Recall that way back at the start of this post I stated the goal was to see if comparing `f32`'s _byte-wise_ was faster doing "proper"`f32` comparisons. Well let's do that.

We'll benchmark how long it takes to build a `BinaryHeap` from both my `OrderedFloat` and the `ordered_float::OrderedFloat::<f32>`.

> As an aside, I recently switched from [criterion](https://github.com/bheisler/criterion.rs) to [divan](https://github.com/nvzqz/divan) for benchmarking due to it's simple and expressive syntax, and I've gotta say I've been loving it ! 🎉
...
use divan::{self, Bencher, counter::BytesCount};
use f32_blogpost_bench::OrderedFloat;
use rand::Rng;
...
fn custom_ord_impl<const N: usize>(bencher: Bencher) {
    bencher
        .with_inputs(|| {
            let mut nums = [f32::default(); N];
            rand::rng().fill(&mut nums);
            let nums_cmp: Result<Vec<_>, &'static str> =
                nums.into_iter().map(OrderedFloat::try_from).collect();

            nums_cmp.unwrap()
        })
        .bench_values(|vec| BinaryHeap::from(vec));
}

#[divan::bench(consts = [10, 100, 1000, 10000])]
fn ordered_float_crate<const N: usize>(bencher: Bencher) {
    bencher
        .with_inputs(|| {
            let mut nums = [f32::default(); N];
            rand::rng().fill(&mut nums);
            let nums_cmp: Vec<_> = nums
                .into_iter()
                .map(ordered_float::OrderedFloat::<f32>)
                .collect();
...
As you can see, by comparing `f32`s via their `&[u8]` representation we're able to fill a `BinaryHeap` twice as quickly on average!


## Conclusion {#conclusion}
I'll be the first to admit that these speedups are relatively small when you consider that [a typical search usually takes O(10ms)](https://qdrant.tech/benchmarks/), but I find it intellectually satisfying that we were able to leverage the structure of our data to squeeze out some speed gains.
...
There's still _much_ work to be done to speed up the vector db further, but we take speedups where we can get them :)
